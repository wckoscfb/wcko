...
async function writeRedirects() {
  // NOTE: no '/*  /index.html  200' catch-all here. Cloudflare's new
  // Workers+Assets pipeline configures SPA fallback automatically via the
  // wrangler.json's `not_found_handling: "single-page-application"` setting,
  // and rejects the explicit catch-all as an "infinite loop" (it sees a rule
  // that would re-trigger itself when the fallback serves index.html).
  // Aliases like /arg or /brasil hit no rule here, get a 404 from the asset
  // server, and are then served the SPA shell — which reads the URL via
  // teamFromSlug() and loads the right team client-side.
  const lines = [
    ...
    '# SPA fallback for everything else is handled by Cloudflare automatically.',
    '',
    ...TEAMS.map(t => `/${t.slug}\t/${t.slug}.html\t200`),
    '',
  ];
  await writeFile(join(DIST, '_redirects'), lines.join('\n'));
  console.log(`  ✓ _redirects updated with ${TEAMS.length} team rules (SPA fallback handled by Cloudflare)`);
}
